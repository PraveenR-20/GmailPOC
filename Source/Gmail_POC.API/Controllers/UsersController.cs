    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromBody] Data.Models.Users user)
        {
            ...
            if (string.IsNullOrWhiteSpace(user.ClientId))
                return BadRequest("Error : Client Id is required");

            if (string.IsNullOrWhiteSpace(user.SecretKey))
                return BadRequest("Error : Secret key is required");

            user.ClientId = Encryption.EncryptSecrets(user.ClientId,_config["EncryptionKey"]);
            user.SecretKey = Encryption.EncryptSecrets(user.SecretKey,_config["EncryptionKey"]);
        ...
        public async Task<IActionResult> Get()
        {
            ...
            {
                u.ClientId = Encryption.DecryptSecrets(u.ClientId,_config["EncryptionKey"]);
                u.SecretKey = Encryption.DecryptSecrets(u.SecretKey,_config["EncryptionKey"]);
            }
            return Ok(users);
        }
        #endregion
